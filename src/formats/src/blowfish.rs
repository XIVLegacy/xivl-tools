...
//!
//! An in-house implementation of the published Blowfish algorithm, whose
//! specification places the cipher in the public domain and whose
//! initialization constants are the hexadecimal expansion of pi's
//! fractional part. The tables below were generated from that expansion,
//! not transcribed from another project's source.
//!
//! One thing here is not the published algorithm and is established
//! against retail data rather than assumed: the two halves of a block are
//! read as **little-endian** 32-bit words, not the big-endian words the
//! specification's test vectors use. Under the big-endian reading not one
//! of the 1155 SQEX files in the install decodes. Under this one all 1155
//! do (`docs/formats/sqex.md`).
//!
//! The key schedule is the published one: the passphrase is consumed four
//! bytes at a time, big-endian, wrapping when it runs out.

use crate::error::{ErrorKind, FormatError, Result};

/// Bytes per block. The container leaves any shorter final run in the
/// clear, so this constant is also what splits an enciphered body from its
/// plaintext tail.
pub const BLOCK_SIZE: usize = 8;

const ROUNDS: usize = 16;

/// A key schedule, expanded once and then used for every block of a file.
pub struct Blowfish {
    p: [u32; 18],
    s: [[u32; 256]; 4],
}
...
impl Blowfish {
    /// Expand a passphrase into a schedule.
    ///
    /// An empty passphrase has no schedule: the published expansion reads
    /// the key cyclically, and an empty key has no cycle. The container's
    /// passphrase is the file's own name, so this failure means a caller
    /// asked for a decode without saying what the file is called.
    pub fn new(key: &[u8]) -> Result<Self> {
        if key.is_empty() {
            return Err(FormatError::new(
                ErrorKind::MissingContainerName,
                0,
                "the key for this container is the file's own name, and none was supplied",
            ));
        }
        ...
        for slot in cipher.p.iter_mut() {
            let mut word = 0u32;
            for _ in 0..4 {
                word = (word << 8) | u32::from(key[index % key.len()]);
                index += 1;
            }
            *slot ^= word;
        }
        ...
        let mut left = 0u32;
        let mut right = 0u32;
        for position in (0..18).step_by(2) {
            (left, right) = cipher.encrypt_words(left, right);
            cipher.p[position] = left;
            cipher.p[position + 1] = right;
        }
        for box_index in 0..4 {
            for position in (0..256).step_by(2) {
                (left, right) = cipher.encrypt_words(left, right);
                cipher.s[box_index][position] = left;
                cipher.s[box_index][position + 1] = right;
            }
        }
        Ok(cipher)
    }
    ...
    fn f(&self, word: u32) -> u32 {
        let a = self.s[0][(word >> 24) as usize & 0xFF];
        let b = self.s[1][(word >> 16) as usize & 0xFF];
        let c = self.s[2][(word >> 8) as usize & 0xFF];
        let d = self.s[3][word as usize & 0xFF];
    ...
    /// Decipher every whole block of `buffer` in place, leaving a shorter
    /// final run untouched. Both halves are little-endian words.
    pub fn decrypt_blocks(&self, buffer: &mut [u8]) {
        self.map_blocks(buffer, |cipher, left, right| {
            cipher.decrypt_words(left, right)
        });
    }
...
/// Initial P array: the first 18 words of the hexadecimal expansion of
/// pi's fractional part, which is what the algorithm specifies. Generated
/// rather than transcribed. `tools/research/census_sqwt.py` recomputes the
/// same expansion.
const P_INIT: [u32; 18] = [
...
/// Initial S boxes: the next 1024 words of the same expansion.
...
mod tests {
    ...
    #[test]
    fn the_constants_are_the_expansion_of_pi() {
        // The first and last words of each table, so a mistyped or
        // truncated table cannot pass. 0x243f6a88 is pi's fractional part
        // in hexadecimal: 0.243f6a88...
    ...
    fn a_block_round_trips() {
        let cipher = Blowfish::new(b"example.form").unwrap();
        let plain = *b"<Window \r\n  <Grid";
        let mut buffer = plain.to_vec();
        cipher.encrypt_blocks(&mut buffer);
        assert_ne!(buffer[..16], plain[..16]);
        // The trailing byte is outside a whole block and stays in the clear.
        assert_eq!(buffer[16], plain[16]);
        cipher.decrypt_blocks(&mut buffer);
        assert_eq!(buffer, plain.to_vec());
    }

    #[test]
    fn a_different_name_is_a_different_schedule() {
        let mut left = b"aligned block!!!".to_vec();
        let mut right = left.clone();
        Blowfish::new(b"one.form")
            .unwrap()
            .encrypt_blocks(&mut left);
        Blowfish::new(b"two.form")
            .unwrap()
            .encrypt_blocks(&mut right);
        assert_ne!(left, right);
    }

    #[test]
    fn an_empty_key_has_no_schedule() {
        let Err(error) = Blowfish::new(b"") else {
            panic!("an empty key produced a schedule");
        };
        assert_eq!(error.kind(), ErrorKind::MissingContainerName);
    }

    #[test]
    fn the_schedule_wraps_a_short_key_rather_than_padding_it() {
        // 72 bytes of key material are consumed. A four-byte key that
        // wraps must differ from the same key zero-padded to 72.
        let mut wrapped = b"aligned block!!!".to_vec();
        let mut padded = wrapped.clone();
        Blowfish::new(b"abcd").unwrap().encrypt_blocks(&mut wrapped);
        let mut long = b"abcd".to_vec();
        long.extend(std::iter::repeat_n(0u8, 68));
        Blowfish::new(&long).unwrap().encrypt_blocks(&mut padded);
