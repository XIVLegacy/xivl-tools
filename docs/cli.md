...
`inspect-command` reads an explicit `command_battle_params.csv` generated by
`xivl-client-data` and queries it by numeric command id or exact
case-insensitive English/Japanese name. YAML is the default for compact agent
...
also identifies the parameter getter's hand argument and the separate command
